...
import com.totsp.bookworm.data.dao.FolderDAO;
...
public class DataManager {
   ...
   public ArrayList<Folder> selectAllFolders (){
       return folderDAO.selectAllFolderNames();
   }

   public ArrayList<Folder> selectFolderByName (String fname) {
      return folderDAO.selectByName(fname);
   }

 /*  public ArrayList<Book> selectBooksByFolder (){
   ...
   public long insertFolder(final Folder f) {
      Log.d("inserting","folder " + f.name);
      long id = folderDAO.insert(f);
      return id;
   }

   public void updateBookFolder(final Book b , long fid ){
       bookDAO.updateFolder(b,fid);
   }
